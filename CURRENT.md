...
**Last updated:** 2026-05-12 (H-004 closed; 53 manifest rows enriched; H-005 queued as next)
...
**Where we are:** Phase 3 preparation continues under the **Path B sequenced-hybrid strategy**. H-004 closed cleanly — the manifest now carries deterministic filename-derived metadata for 53 of 157 items (39 DOW + 2 DOS + 12 NASA), four new columns appended (`country`, `date`, `date_precision`, `item_type`) joined to the pre-existing `agency` column updated in place.
...
**H-004 surfaced two open follow-up items** that the researcher should decide:

1. **D48 and D49** — `DOW-UAP-D48-Report-September-1996.pdf` and `DOW-UAP-D49-Launch-Summary-February-2000.pdf`. Both have known DOW item_types and parseable dates but no location token in the filename. Currently unparsed. Options: refine the regex to allow empty `country` (H-004a, deterministic), or open the PDFs for content-based location extraction (later sub-phase, LLM-assisted with researcher review). Not blocking H-005 or Phase 3.

2. **D54 literal-`NA` date sentinel** — `DOW-UAP-D54-Mission-Report-Mediterranean-Sea-NA.pdf` has blank `date` and `date_precision` by design (the source filename's date position is the literal token `NA`; defaulting would fabricate provenance). Documented behavior, no fix required, but downstream consumers must respect the `date_precision` column when reading dates.

**Phase status:** Phase 0 ✓. Phase 1 ✓. Phase 2 ✓. Phase 3 prep partial (unit decision settled; H-004 closed; H-005 still owed). Phase 3 design (taxonomy itself) gated by H-005 (external-taxonomy survey, to run in Eurydice chat) and the Phase 3 reading-as-cases pass.
...
2. **Phase 3 — Taxonomy design.** Once H-005 produces the external-taxonomy reference, read the clean ~53 DOW/DOS/NASA cases (using the filename-derived metadata as the index), extract incidents, observe patterns, draft taxonomy v1 grounded in the actual archive.
3. **Phase 4 — Schema.** Implement the two-tier model in SQLite.
4. **Phase 5 v1 — Index and classify the clean subset.** End of Path B v1.
5. **Later (post-v1):** war.gov page parse (refines `source_url`); FBI case-file extraction with LLM assistance + researcher review; DOD video segmentation; H-004a refinement for D48/D49 if researcher chooses. Taxonomy versions to v2 and beyond as messier data lands.
