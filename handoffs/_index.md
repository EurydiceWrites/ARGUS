...
| ID | Date | One-line summary |
|---|---|---|
| [H-004](H-004_filename_extractor.md) | 2026-05-12 | Filename metadata extractor. `src/extract_filename_metadata.py` v1.0.0 parses DOW / DOS / NASA filename conventions into four new manifest columns (`country`, `date`, `date_precision`, `item_type`) plus an in-place `agency` update. **53 of 157 rows enriched** (39 DOW + 2 DOS + 12 NASA). 24 unit tests pass; idempotency byte-identical. 0 agency disagreements with the H-002 first-pass tagging. 2 candidates surfaced for future refinement (DOW D48 + D49 — filename lacks location). 1 design-preserving blank (DOW D54 literal-`NA` date sentinel; defaulting would fabricate provenance). FBI / DOD-video / USPER rows untouched in the new columns. |
